...
RUN python -m venv /opt/sql-flatmap-env
ENV PATH /opt/sql-flatmap-env/bin:$PATH
ENV VIRTUAL_ENV /opt/sql-flatmap-env

# Set the working directory.
WORKDIR /workspaces/sql-flatmap/

# Install the run time Python dependencies in the virtual environment.
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/sql-flatmap/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/sql_flatmap/ && touch src/sql_flatmap/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/sql-flatmap/
